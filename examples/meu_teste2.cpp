...
void arquivoErroRelativo(double xi, double ve, double va, double *somaER)
{
    ofstream erroRelativo;
    erroRelativo.open("erroRelativo.csv", ios_base::app);

    double ER = va - ve;

    *somaER = *somaER + (ER * ER);

    erroRelativo << xi << "," << ve << "," << va << "," << ER << "," << ER * ER << endl;
...
void calcular(const char **argv)
{
    ...
    */
    ...
    double stepSize = (b - a) / (sol_size + 1);
    ...
    cout << "STEP"
         ...
         << "V. APROX"
         << "\t" << endl;
    ofstream erroRelativo, normaL2;

    erroRelativo.open("erroRelativo.csv");
    erroRelativo << "x,valor exato,valor aproximado, ER,ER²" << endl;
    erroRelativo << "-1,2,2,0,0" << endl;
    erroRelativo.close();

    double somaER = 0; // soma dos quadrado do erro relativo
    for (int i = 0; i < sol_size; ++i)
    {
        step += stepSize;
        valorExato = (1 + step * step); // u = 1+x²
        ER = data[i] - valorExato;

        arquivoErroRelativo(step, valorExato, data[i], &somaER);
        cout << step << "\t\t" << valorExato << "\t\t" << data[i] << "\t\t" << ER << endl;
    }

    somaER = somaER * stepSize; // pela fórmula do slide tenho que multiplicar pelo h
    cout << "SOMA ER²: " << somaER << endl;
    cout << "RAIZ ER²: " << sqrt(somaER) << endl;
    ...
    normaL2.open("normaL2.csv", ios_base::app);
    normaL2 << "x,y" << endl;
    normaL2 << sol_size << "," << sqrt(somaER) << endl;
    normaL2.close();
